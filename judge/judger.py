...
def checkCaseInput(case):
    "Checks if the input is compatible with the capability of the judger"
...
def getLawNum(law_id):
    "Takes the mongoDB reference of the law and returns a law number"
    law = db_laws.find_one({'_id': ObjectId(law_id)})
    return law["num"]

def getLawInfo(law_id):
    "Takes the mongoDB reference of the law and returns a law number"
    law = db_laws.find_one({'_id': ObjectId(law_id)})
    return (law["num"], law["title"])

def getRelevantLaws(case):
    "For a given case, returns all the relavent laws"
    act = case["Act"]
    ...
    for lawID in relevantLawIDs:
        #lawID[1] is the confidence score
