...
for dict_filename in os.listdir('dicts'):
    with gopen('dicts/'+dict_filename, 'rt') as f:
        temp = f.readlines()
        save_len = len(temp)
...
start = time.time()
model = gensim.models.KeyedVectors.load_word2vec_format(model_folder + '/' + model_filename, binary=True)
...
start = time.time()
model = gensim.models.KeyedVectors.load_word2vec_format(model_folder + '/' + slim_filename, binary=True)
